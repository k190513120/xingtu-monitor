...
// 处理单个博主：获取 kolId（优先用缓存）、名片、视频、报价
...
export async function executeTask(config: TaskConfig, kv: KVNamespace): Promise<void> {
  ...
  try {
    ...
    // ── 确保博主表中有星图相关字段（不存在则自动创建）──────────────────
    await updateStatus(kv, {
      state: 'running',
      message: `共 ${bloggerEntries.length} 个博主，正在检查/创建字段...`,
    ...
    // ── 第二步：并发处理所有博主 ────────────────────────────────────────
    const allVideoFields: Record<string, any>[] = [];
    const allErrors: string[] = [];
    let processed = 0;
    let errorCount = 0;
    const tikhub = new TikHubClient(config.tikHubApiKey);

    for (let i = 0; i < bloggerEntries.length; i += CONCURRENCY) {
      const batch = bloggerEntries.slice(i, i + CONCURRENCY);
      ...
      await updateStatus(kv, {
        state: 'running',
        message: `进度 ${processed}/${bloggerEntries.length}（${errorCount} 个失败），已获取 ${allVideoFields.length} 条视频${errorSuffix}`,
        totalCount: bloggerEntries.length,
        processedCount: processed,
        videoCount: allVideoFields.length,
        lastRunAt: Date.now(),
      });

      if (i + CONCURRENCY < bloggerEntries.length) {
        await new Promise(r => setTimeout(r, BATCH_DELAY));
      }
    }

    // ── 第三步：写入目标多维表格 ────────────────────────────────────────
    if (allVideoFields.length > 0) {
      await updateStatus(kv, {
        state: 'running',
        message: `正在写入 ${allVideoFields.length} 条视频数据到目标表...`,
        lastRunAt: Date.now(),
      });
    ...
    await updateStatus(kv, {
      state: 'done',
      message: `✅ 完成！处理博主 ${processed} 个（${errorCount} 个失败），写入视频 ${allVideoFields.length} 条${finalErrorInfo}`,
      totalCount: bloggerEntries.length,
      processedCount: processed,
      videoCount: allVideoFields.length,
      lastRunAt: Date.now(),
